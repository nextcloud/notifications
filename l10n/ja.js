OC.L10N.register(
    "notifications",
    {
    "We want to keep offering our push notification service for free, but large number of users overload our infrastructure. For this reason we have to rate-limit the use of push notifications. If you need this feature, consider using Nextcloud Enterprise." : "私たちはプッシュ通知サービスを無料で提供し続けたいのですが、多数のユーザーが私たちのインフラに負荷をかけます。このため、プッシュ通知の使用を制限する必要があります。この機能が必要な場合は、Nextcloud Enterpriseのご利用をご検討ください。",
    "User not found" : "ユーザーが見つかりません",
    "An unexpected error occurred, ask your administration to check the logs." : "予期しないエラーが発生しました。ログを確認するよう管理者に依頼してください。",
    "Hello %s" : "こんにちは %s さん",
    "Hello %s," : "こんにちは %s さん、",
    ...
    "Web push settings" : "Webプッシュ通知の設定",
    "Web push is a push notification system that lets web applications send notifications to users´ mobile apps or browser, even when the app is not open in a tab." : "Webプッシュとは、Webアプリケーションが、ユーザーのモバイルアプリやブラウザに対して、アプリがタブで開かれていない場合でも通知を送信できるプッシュ通知システムです。",
    "Web push notifications are encrypted but routed through services provided by Mozilla, Microsoft, Apple, Google or user-choosen ones in case of mobile apps. While the content is protected, metadata such as timing and frequency of notifications may be exposed to these providers." : "Webプッシュ通知は暗号化されていますが、モバイルアプリの場合、Mozilla、Microsoft、Apple、Google、またはユーザーが選択したサービスを経由して配信されます。通知の内容は保護されていますが、通知のタイミングや頻度などのメタデータは、これらのプロバイダーに開示される可能性があります。",
    "Allow web push to be used" : "Webプッシュ通知の使用を許可する",
    "Allow web push for browsers" : "ブラウザでWebプッシュ通知を許可する",
    "None" : "なし",
