OC.L10N.register(
    ...
    "Web push settings" : "Web 推送設定",
    "Web push is a push notification system that lets web applications send notifications to users´ mobile apps or browser, even when the app is not open in a tab." : "Web 推送是一種推送通知系統，讓網頁應用程式可以向用戶的手機應用程式或瀏覽器發送通知，即使該應用程式未在分頁中開啟亦可。",
    "Web push notifications are encrypted but routed through services provided by Mozilla, Microsoft, Apple, Google or user-choosen ones in case of mobile apps. While the content is protected, metadata such as timing and frequency of notifications may be exposed to these providers." : "Web 推送通知會經加密，但會透過 Mozilla、Microsoft、Apple、Google 或（在手機應用程式情況下）用戶自選的服務提供者轉送。雖然內容受到保護，但通知的時間及頻率等元資料可能會暴露給這些服務提供者。",
    "Allow web push to be used" : "允許使用 Web 推送",
    "Allow web push for browsers" : "允許瀏覽器使用 Web 推送",
    "None" : "無",
